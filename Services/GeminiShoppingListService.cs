...
/// <summary>
/// Service for generating shopping lists using Google Gemini AI
/// </summary>
public class GeminiShoppingListService : IShoppingListService
{
    private readonly GoogleAI _genAi;
    private readonly GenerativeModel _model;

    public GeminiShoppingListService(string apiKey, string modelName = "gemini-2.5-flash")
    {
        _genAi = new GoogleAI(apiKey);
        _model = _genAi.GenerativeModel(model: modelName);
        _model.Timeout = TimeSpan.FromMinutes(2);

        Console.WriteLine($"✅ GeminiShoppingListService zainicjalizowany ({modelName})");
    }
